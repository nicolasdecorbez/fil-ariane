...
import com.example.ardianethread.Services.Singleton
...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!Global.Current.isLoged) {
            ...
            if (!Global.TempFriendUser.idDefined) {
                Global.Current.User.id?.let { FriendServices.Friend.GetAllFriends(it, this) }
                val intent = Intent(this, MainActivity::class.java)
                startActivity(intent)
            }
            if (Global.TempFriendUser.idDefined && !Global.TempFriendUser.arrayDefined ) {
                if (Global.TempFriendUser.iterator < Global.TempFriendUser.Ids.size) {
                    FriendServices.Friend.SetFriendsArray(Global.TempFriendUser.Ids[Global.TempFriendUser.iterator],
                       this)
                    val intent = Intent(this, MainActivity::class.java)
                    startActivity(intent)
                } else {
                    Global.TempFriendUser.arrayDefined = true
                    val intent = Intent(this, MainActivity::class.java)
                    startActivity(intent)
                }
                Global.TempFriendUser.iterator++
            }
            Log.e("Global User", Global.TempFriendUser.UsersArray.toString())
